...
expName = 'TCS_WM_Bloque1'  # filename
...
def saveExcel():
    ...
    writer = ExcelWriter('data' + os.path.sep + '%s_%s_Bloque1.xls' %(expInfo['ParticipantNumber'], expInfo['date']))
    df.to_excel(writer,'Bloque1',index=False)
    writer.save()
...
#'Ovxxxxx' : set stimulation speed in 1/100 degrees per seconds ('s'='0'(all areas) or '1' to '5', 'xxxxx'='00001' to '99999')
#'Otxxxx'  : set stimulation temperature in 1/100 degrees ('s'='0'(all areas) or '1' to '5', 'xxxx'='0001' to '6000')
#'Oe'      : display currEnt temperatures for neutral and area 1 to 5 in 1/100 degrees
#'B'       : display Battery voltage and % of charge
#'Yxxxx'   : set the duration of temperature display during stimulation in ms ('xxxx'='0000' to '9999')
...
#'Wr'      : display temperature profile, if defined, for each area
#'Wwabcdennnndddtttdddttt...' : load a temperature profile defined by segments of variable duration
#            'abcde': areas defined ('11111' all areas, '10000' just area 1, ...), 'nnnn': number of segments ('0001' to '5000')
#            'dddttt': list of duration of segment in ten ms ('001' to '999') and temperature at end of segment ('000' to '600')
#<!>When sending command, ensure a minimum of 1ms between each caracters to avoid loss of caracters in serial com
