...
temp = 42.2 # temperature (a decimal number e.g.: 47.0)
vibration = 120 # intensity of the vibration (an integer value from 0 to 120)


try:
    ser = serial.Serial('/dev/cu.usbmodem1410', 115200)
except:
    ser=False
...
# EXCEL
def saveExcel(blockTypes,vas1,vas2,vas3,rts1,rts2,rts3,vas1t,vas2t,vas3t,handsused,timestamps,suffix):
    ...
    for idx,blockType in enumerate(blockTypes):
        ...
        ws['L'+str(idx+2)] = timestamps[idx]
    wb.save(filename = dest_filename)

def quit_task(suffix):
    saveExcel(BLOCKTYPES,VAS1,VAS2,VAS3,RTS1,RTS2,RTS3,VAS1T,VAS2T,VAS3T,HANDS,TIMESTAMPS,suffix)
...
# for safety, create a csv file that will be written in each row
onlineCSV = 'DataTSSP' + os.path.sep + '%s_%s_OnlineData.csv' %(expInfo['1 SubjectNumber'],expInfo['Date'])
with open(onlineCSV, 'w') as onlineFile:
    write_online = csv.writer(onlineFile)
    write_online.writerow(['Block','VAS1','VAS2','VAS3','HandUsed', 'TargetTemp','LastTempRead','S2TimeStamp'])
...
for idx, block in enumerate(blocks):
    ...
    while t1<29.998:
        ...
        if t1>trial_dur*cont-0.002: # send trigg each 3 secs
            ...
            blockStimTimes.append(globalClock.getTime())
            cont+=1
            # timestamp for the EDA device
            BlockTimeStamps.append(datetime.timestamp(datetime.now()))
        if t1>temp_fs*tempcont: # read temp each 500 ms
            tempcont+=1
            read_temp()
        core.wait(.002)
    ...
    t1 = blockClock.getTime()
    blockStimTimes.append(globalClock.getTime())
    TIMESTAMPS.append(str(BlockTimeStamps[0]))
    while t1<rtime+31.5:
        t1 = blockClock.getTime()
        if t1>temp_fs*tempcont: # read temp each 500 ms
            tempcont+=1
    ...
    # write online CSV data
    with open(onlineCSV, 'a') as onlineFile:
        write_online = csv.writer(onlineFile)
        write_online.writerow([BLOCKTYPES[-1],VAS1[-1],VAS2[-1],VAS3[-1],HANDS[-1],temp,all_temps[-1][0],BlockTimeStamps])
    while t1<55:
        core.wait(.005)
        t1 = blockClock.getTime()
        if t1>temp_fs*tempcont: # read temp each 500 ms
